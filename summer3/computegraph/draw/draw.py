...
from summer3.computegraph import options
from summer3.computegraph.types import Variable, Function, Data
from summer3.computegraph.utils import assign
...
def draw_compute_graph_mpl(
    dag: nx.DiGraph, targets: Optional[List[str]] = None, **kwargs
):
...
def draw_computegraph_plotly(
    ...
    def get_label_and_desc(name, node_spec):
        ...
        if isinstance(node_spec, Function):
            ...
            out_text += ["args:"]
            out_text += [f"{tab_str}{arg}"[:max_label] + "\n" for arg in node_spec.args]
            out_text += ["kwargs:"]
            out_text += [
                f"{tab_str}{k}: {v}"[:max_label] + "\n"
